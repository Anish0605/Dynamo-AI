...
async def vision(message: str = Form(...), file: UploadFile = File(...)):
    ...
    try:
        ...
        res = client.chat.completions.create(model="llama-3.2-11b-vision-preview", messages=[{"role":"user","content":[{"type":"text","text":message},{"type":"image_url","image_url":{"url":f"data:{file.content_type};base64,{b64}"}}]}])
...
async def transcribe(file: UploadFile = File(...)):
    if not client: raise HTTPException(500, "Key Missing")
    try:
        with open("t.wav", "wb") as f: f.write(await file.read())
        with open("t.wav", "rb") as f: tx = client.audio.transcriptions.create(model="whisper-large-v3-turbo", file=f, response_format="text")
        return {"text": tx}
    except Exception as e: raise HTTPException(500, str(e))
